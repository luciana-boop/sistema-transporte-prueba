...
export class CuentasController {
  ...
  async registrarMovimiento(req: Request, res: Response): Promise<void> {
    try {
      const { cuentaId, tipo, monto, monedaId, tipoPagoId, concepto, referencia, cuentaDestinoId, fecha } = req.body;
      ...
      if (!['INGRESO', 'EGRESO', 'TRANSFERENCIA'].includes(tipo)) {
        R.badRequest(res, 'tipo debe ser INGRESO, EGRESO o TRANSFERENCIA'); return;
      }
